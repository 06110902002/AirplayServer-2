...
class AppleService implements DNSSDService, Runnable {
    ...
    public void run() {
        while (true) {
            ...
            synchronized (this) {
                if (fNativeContext == 0)
                    break;    // Some other thread stopped our DNSSD operation; time to terminate this thread
                if (result == 0)
                    continue;        // If BlockForData() said there was no data, go back and block again
                result = ProcessResults();
                if (fNativeContext == 0)
                    break;    // Event listener stopped its own DNSSD operation; terminate this thread
                if (result != 0) {
